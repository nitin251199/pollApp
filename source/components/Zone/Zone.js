...
export default class Zone extends Component<Props> {
 ...
 checkInternet = async () => {
  ...
  get_data_from_live = async () => {
    ...
    {
      let error_msg= "Internet is not connected with device, please connect internet and come again on this screen.";
      this.displayToastMessage(error_msg);
    }
...
get_local_zones = async () => {
    ...
    if (zone_data !== null) {
      ...
      {
        let obj = { label: zone_data[i].name , value: zone_data[i].id, color : '#000000' };
    ...
    return (
      ...
      <Loader loading={this.state.loading} />
        ...
        <ScrollView>
          ...
          <View style={{ margin: 15,padding:10,backgroundColor:'white',borderRadius:5, elevation:2 }}>
            <Text style={{ fontSize:18,color:'red',fontWeight:'bold' }}>
            Please check your internet settings , you are not connected with internet, please connect internet and come on thid screen again.
            </Text>
          </View>
          :
          null
...
<View style={{ margin: 15,padding:10,backgroundColor:'white',borderRadius:5, elevation:2 }}>
            ...
            Please select Zone (विधानसभा क्षेत्र ) to change App data, and press "Save Zone" button.
              </Text>
          </View>
          ...
          <RNPickerSelect
                      ...
                      onValueChange={(zone_id) => {  this.setState({ zone_id : zone_id })   } }
        ...
        <View style={styles.footer}>
          ...
          <Button
            onPress={() => { this.get_data_from_live() }}
