...
import { Text, View, Image,FlatList,TextInput, Animated, ImageBackground, TouchableOpacity,StyleSheet, ScrollView,PermissionsAndroid,Platform,Alert,Dimensions } from 'react-native';
      ...
      else
      {
        ...
        for(let i=0;i<zc_accounts.length;i++)
        {
          zc_accounts_list.push(
            ...
            <Text style={{fontWeight:'bold',width:this.state.width*0.25}}>{zc_accounts[i].name}</Text>
            <Text style={{fontWeight:'bold',width:this.state.width*0.25}}>{zc_accounts[i].phone_number}</Text>
            <Text style={{fontWeight:'bold',width:this.state.width*0.25,textAlign:'center'}}>{zc_accounts[i].uploaded_sample}</Text>

            <View style={{flexDirection:'row'}}>
        <Button
         onPress={() => {  this.props.navigation.navigate('Editaccount',{member_data:zc_accounts[i]}) }}
          contentStyle={{ backgroundColor: 'green',height:35 }}
          icon="pencil" mode="contained">
        </Button>

        <Button
         onPress={() => { this.alert_for_delete(zc_accounts[i].mem_id)  }}
      ...
      if( response.success == 1 )
      {
         await AsyncStorage.removeItem('zv_accounts_data');
          ...
          <Button
           onPress={() => {  this.props.navigation.navigate('Editaccount',{member_data:zc_accounts[i]}) }}
            contentStyle={{ backgroundColor: 'green',height:35 }}
            icon="pencil" mode="contained" compact>
          </Button>

          <Button
           onPress={() => { this.alert_for_delete(zc_accounts[i].mem_id)  }}
            contentStyle={{ backgroundColor: 'red',height:35 }}
            icon="delete" mode="contained" compact>
          </Button>
          </View>
  ...
  alert_for_delete = async (mem_id) => {
    Alert.alert(
      ...
      [
        ...
        {
          text: "Yes, Delete it",
          onPress: () => { this.delete_record(mem_id) },
          style: "cancel",
        }
    ...
    return (
      ...
      <Loader loading={this.state.loading} />
        ...
        <ScrollView>
          ...
          <View  style={{ alignItems: 'center', marginBottom: 10,flexDirection:'row',justifyContent:'space-between',borderBottomWidth:1,borderBottomColor:'#ddd',padding:15 }} >
              <Text style={{fontWeight:'bold',width:this.state.width*0.25}}>Name</Text>
              <Text style={{fontWeight:'bold',width:this.state.width*0.25}}>Phone</Text>
              <Text style={{fontWeight:'bold',width:this.state.width*0.25,textAlign:'center'}}>Uploaded</Text>
              <Text style={{fontWeight:'bold',width:this.state.width*0.25,textAlign:'center'}}>Actions</Text>
