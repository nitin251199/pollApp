...
import { Text, View, Image,TextInput, Animated, ImageBackground, TouchableOpacity,StyleSheet, ScrollView,PermissionsAndroid,Platform,Alert  } from 'react-native';
...
export default class Question extends Component<Props> {
 ...
 checkInternet = async () => {
  ...
  getUserLocation = async () => {
        ...
        Geolocation.getCurrentPosition(
            (position) => {
                ...
                let lat_long_str = position.coords.latitude+','+position.coords.longitude;
                this.setState({
  ...
  save_image = async (image_data) => {
      ...
      let photo_img = <Image  style={{width: 350,height: 400,resizeMode:'contain'}}  source={{uri:image_data.path}} />
      this.setState({
        photo_img : photo_img,
          ...
          for(let j=0;j<pre_val_arr.length;j++)
              {
                ...
                new_selected_option_id = new_selected_option_id+','+pre_val_arr[j];
              }
        }
        else
        {
  ...
  re_render_question = async () => {
      ...
      for( let i=0; i<question_data.questions.length;i++ )
      {
          ...
          if(  question.question_type == 'normal' || question.question_type == 'dynamic_choice')
          {
              ...
              let radio_group = <RadioButton.Group key={question.id} value={question.selected_option_id}  onValueChange={(value) => { this.set_answer_obj(value,question,i) }}   >{options_html}</RadioButton.Group>
          ...
          if(  question.question_type == 'rating')
          {
              ...
              for(let j=0;j<options.length;j++)
              {
                ...
                options_html.push(
                  <TouchableOpacity key={j} style={styles.mojilist} onPress={() => { this.set_answer_obj(options[j].id,question,i) }}>
                    {
              ...
              for( let m=0;m<child_questions.length;m++)
              {
                ...
                for(let j=0;j<options.length;j++)
                {
                  ...
                  options_html.push(
                    ...
                    <TouchableOpacity  onPress={( ) => { this.set_answer_inner_obj(options[j].id,child_questions[m],m,i) }} style={button_option_class}>
          ...
          if(  question.question_type == 'multi_choice')
          {
              ...
              for(let j=0;j<options.length;j++)
              {
                ...
                options_html.push(
                  <Checkbox.Item   status={option_status} label={options[j].option_title}  onPress={() => { this.set_answer_obj_multiple(options[j].id,question,i) }} />
                )
              }
              questions_html_arr.push(
                  <Card style={card_class} key={i}>
              ...
              for( let m=0;m<child_questions.length;m++)
              {
                ...
                for(let j=0;j<options.length;j++)
                {
                  ...
                  options_html.push(
                    ...
                    <TouchableOpacity  onPress={() => { this.set_answer_obj(options[j].id,question,i) }} style={button_option_class}>
  ...
  _renderButton = (text, onPress) => (
    <TouchableOpacity onPress={onPress} style={{backgroundColor:'#ddd', position:'absolute',right:10,top:10,width:30,height:30,borderRadius:50,alignItems:'center',justifyContent:'center',zIndex:99,}}>
      <View style={{}}>
    ...
    if (question_data !== null) {
      ...
      for( let i=0; i<question_data.questions.length;i++ )
      {
          ...
          if(  question.question_type == 'multiple')
          {
              ...
              for( let m=0;m<child_questions.length;m++)
              {
                ...
                for(let j=0;j<options.length;j++)
                {
                  ...
                  <TouchableOpacity  onPress={() => { this.set_answer_inner_obj(options[j].id,child_questions[m],m,i) }} style={styles.button_option}><Text style={styles.button_option_text}>{options[j].option_title}</Text></TouchableOpacity>
          ...
          if(  question.question_type == 'multi_choice')
          {
              ...
              {
                options_html.push(
                  <Checkbox.Item  value={options[j].id} label={options[j].option_title}  onPress={() => { this.set_answer_obj_multiple(options[j].id,question,i) }} />
                )
              }
              questions_html_arr.push(
                  <Card style={card_class} key={i}>
          ...
          if(  question.question_type == 'single_choice')
          {
              ...
              for( let m=0;m<child_questions.length;m++)
              {
                ...
                for(let j=0;j<options.length;j++)
                {
                  ...
                  options_html.push(
                    //<RadioButton.Item value={options[j].id}  style={styles.radiolist} label={options[j].option_title} />
                    <TouchableOpacity  onPress={() => { this.set_answer_obj(options[j].id,question,i) }} style={styles.button_option}>
                      <Text style={styles.button_option_text}>{options[j].option_title}</Text>
                    </TouchableOpacity>
                  )
                }
...
next_question = async () => {
  ...
  if( question_type == 'normal' || question_type == 'rating' || question_type == 'single_choice' || question_type == 'dynamic_choice'   )
  {
  ...
  {
      let child_questions = question_data.questions[active_index].child_questions;
...
save_data_local_old = async () => {
    ...
    {
      if( questions[i].question_type == 'normal'  || questions[i].question_type == 'rating' || questions[i].question_type == 'multi_choice'  || questions[i].question_type == 'single_choice' || questions[i].question_type == 'dynamic_choice' )
      {
          let survey_data_obj = {
...
{
  let error_msg =   'Please check did you enter name, mobile number and photo before submit .';
  this.displayToastMessage(error_msg);
}
...
upload_image_option = async () => {
  Alert.alert(
    ...
    [
      ...
      {
        text: "From Gallery",
        onPress: () => { this.upload_image_gallery() },
        style: "cancel",
      },
      {
        text: "From Camera",
        onPress: () => { this.upload_image_camera() },
        style: "cancel",
      },
    ],
    {
    ...
    return (
      ...
      <ScrollView>
        <View style={{ paddingHorizontal: 20, flex: 1 }}>
          ...
          <View style={{ alignItems: 'center', marginBottom: 20 }}>
            <Image style={styles.logo} source={require('../../images/logo.png')} />
            <Text style={{ fontWeight: 'bold', }}>सर्वे : <Text style={{ color: '#000000' }}>2021/10</Text></Text>
          </View>
          ...
          <View style={{ marginBottom: 15 }}>
            <Button
              theme={{ roundness: 0 }}
              contentStyle={{ flexDirection: 'row-reverse', backgroundColor: '#c48404', height: 55, }}
              icon="map" mode="contained"   onPress={( ) =>  this.getUserLocation() }   >
          ...
          <View style={{ marginBottom: 15 }}>
            <Button
              ...
              icon="camera-plus-outline" mode="contained"   onPress={( ) =>  this.upload_image_option() }   >
      ...
      <View style={styles.footer}>
        ...
        <Button
          onPress={() => { this.save_all_survey_data() }}
          contentStyle={{ flexDirection: 'row-reverse', backgroundColor: '#25904f',height:50 }}
