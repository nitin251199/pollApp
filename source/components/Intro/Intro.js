...
import { Text, View, Image, Animated, ImageBackground,TouchableOpacity, StyleSheet,Dimensions, ScrollView,Platform,Alert } from 'react-native';
import { Card, Title, Checkbox, Button, DefaultTheme, Provider as PaperProvider, Appbar, RadioButton, TextInput } from 'react-native-paper';
...
check_user_type = async () => {
  ...
  if(user_profile!=null)
  {
    ...
    if(user_profile.member_role == 'QC' || user_profile.member_role == 'ZC' )
    {
...
sync_local_to_server = async () => {
    ...
    for( let i=0; i<poll_data.length; i++ )
    {
      ...
      {
        if( questions[k].question_type == 'normal'  || questions[k].question_type == 'rating' || questions[k].question_type == 'multi_choice'  || questions[k].question_type == 'single_choice' || questions[k].question_type == 'dynamic_choice' )
        {
            let survey_data_obj = {
...
zone_change_error = async () => {
  ...
  let error_msg =   'Zone is already set, you can not change zone, please ask to ZC to change your zone.';
  this.displayToastMessage(error_msg);
...
refresh_question_zone_data = async (user_data) => {
  ...
  console.log('params',params);
    ...
    if( response.success == 1 )
    {
        ...
        let error_msg =  'Zone data is synced from live, you can use App now for survey.';
        this.displayToastMessage(error_msg);
    }
    else
    {
...
alert_for_refresh = async () => {
  ...
  render() {
    ...
    return (
      ...
      <ImageBackground source={require('../../images/bg.jpeg')} resizeMode="contain" style={{}}>

      <View style={styles.header}>
      <View>
      <Image resizeMode="contain" style={styles.logo} source={require('../../images/logo.png')} />
      </View>
                        <TouchableOpacity  onPress={() => {  this.props.navigation.navigate('Notifications') }}  style={{position:'relative'}}>
                        {
                            this.state.new_count > 0 ?
                          <View style={styles.noti}>

                            <Text style={{fontWeight:'bold',color:'#fff',fontSize:14}}>{this.state.new_count}</Text>

                            </View>
                            :
                            null
                          }
      <Icon name="ios-notifications" size={40} iconStyle={{marginBottom:20}} color="#666" />
      </TouchableOpacity>
      ...
      <Text style={[styles.boxtext,{color:'#000000'}]}>Inserted Records</Text>
      <Text style={[styles.boxtext,{color:'green'}]}> {this.state.total_sample}</Text>
      </TouchableOpacity>
      ...
      <TouchableOpacity  onPress={() => { this.props.navigation.navigate('Question') }} style={styles.box}>
      <Icon name="stats-chart" size={45} color="#c84006" />
      <Text style={[styles.boxtext,{color:'#000000'}]}>नया सैंपल</Text>
      </TouchableOpacity>
      :
      <TouchableOpacity  onPress={() => {  this.app_not_ready() }} style={styles.box}>
      <Icon name="stats-chart" size={45} color="#c84006" />
      <Text style={[styles.boxtext,{color:'#000000'}]}>नया सैंपल (Not ready)</Text>
      </TouchableOpacity>
  }

      <TouchableOpacity onPress={() => { this.try_sync_data_server() }} style={styles.box}>
      <Icon name="cloud-upload" size={45} color="#008d0e" />
      <Text style={[styles.boxtext,{color:'#000000'}]}>अपलोड करें Pending-{this.state.total_pending}</Text>
      </TouchableOpacity>


      <TouchableOpacity  onPress={() => {  this.props.navigation.navigate('Pendinglist') }} style={styles.box}>
      <Icon name="hourglass" size={45} color="#fb8500" />
      <Text style={[styles.boxtext,{color:'#000000'}]}>Review Pending</Text>
      <Text style={[styles.boxtext,{color:'red'}]}> {this.state.total_pending}</Text>
      </TouchableOpacity>
      </>
      :
      null
...
<>
{
     ...
     <>
     <TouchableOpacity   onPress={() => { this.props.navigation.navigate('Members') }} style={styles.box}>
     <Icon name="people" size={45} color="#fb8500" />
     {
                this.state.member_role == 'QC' ?
                <Text style={[styles.boxtext,{color:'#000000'}]}> Z.C. Accounts : {this.state.zc_count} </Text>
              :
              <Text style={[styles.boxtext,{color:'#000000'}]}> O.T. Accounts : {this.state.zc_count} </Text>
              }
...

      {
        ...
        style={{marginBottom:20,backgroundColor:'#021561',paddingVertical:15}}  icon="sync-circle" mode="contained" onPress={() => { this.alert_for_refresh() }} >
            <Text style={{fontSize:20}}> Zone: {this.state.user_zone_data.name} ( {this.state.user_zone_data.code} )</Text>
          </Button>
          :
          <Button
          labelStyle={{fontSize: 30}}
        ...
        <Button labelStyle={{fontSize: 35}} style={{marginBottom:20,backgroundColor:'red',paddingVertical:15,}} icon="disc" mode="contained"  onPress={() => { this.logout() }}>
